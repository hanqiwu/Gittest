...
def check_exit_return(tempfile): #No 'exit' is allowed in any function. and return is mandatory in every function.
    ...
    with open(tempfile, 'r') as openfile:
       ...
       #logger.debug ("Current line is :" + line)
           ...
           if re.search(r'\s*\w*.*\(.*\)\s*', line) and not re.search(r'^\s*#.*', line) and not re.search(r'\s*if.*', line) and not re.search(r'\s*for.*', line) and not re.search(r'\s*while.*', line) and func_mark == 0:  #Find definition of function and the line is not commented.
